...
document.getElementById('btn-generate').addEventListener('click', async () => {
    ...
    try {
        ...
        // 渲染區塊 (新增頁尾顯示模型資訊)
        // [修正] 使用 escapeHtml 包覆內容，防止 XSS 與吃字問題
        // 注意：先 escape 再 replace 換行，順序不能錯
        storyContent.innerHTML = `
            <div style="background:#f0f2f5; padding:15px; border-radius:8px; margin-bottom:15px; font-size:0.95rem; line-height:1.6;">
                ...
                <div>${escapeHtml(generatedResult.settings_list).replace(/\n/g, '<br>')}</div>
            </div>

            <div style="text-align:center; margin: 30px 0 20px 0;">
                <span style="font-size:1.5rem; font-weight:bold; color:#5e6b75; line-height:1.4;">《${escapeHtml(generatedResult.story_title)}》</span>
            </div>

            <h3>📐 三幕劇結構</h3>
            <p>${escapeHtml(data.three_act_structure || '').replace(/\n/g, '<br>')}</p>
            <hr>

            <h3>⭕ 故事圈 (Story Circle)</h3>
            <p>${escapeHtml(data.story_circle || '').replace(/\n/g, '<br>')}</p>
            <hr>

            <h3>📖 劇情大綱</h3>
            <p>${escapeHtml(data.story_outline || '').replace(/\n/g, '<br>')}</p>
            <hr>
            ...
            <div style="margin-top: 30px; text-align: center; color: #bfaea8; font-size: 0.8rem; border-top: 1px dashed #eee; padding-top: 10px;">
                Generated by ${escapeHtml(generatedResult.used_model || 'Unknown AI')}
            </div>
        `;
    } catch (e) {
        loading.style.display = 'none';
...
document.getElementById('btn-save').addEventListener('click', async () => {
    ...
    const newStory = {
        ...
        used_model: generatedResult.used_model // [新增] 儲存使用的模型資訊
    };
...
async function renderHistory(searchQuery = '') {
    ...
    stories.forEach(story => {
        ...
        // [新增] 讀取模型名稱
        const modelName = story.used_model || 'Unknown AI';

        // [修正] 使用 escapeHtml 包覆變數，確保安全顯示
        item.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                <div class="history-header-area" style="cursor:pointer; flex:1;">
                    <div style="font-weight:bold; font-size:1.1rem; color:#5e6b75;">${escapeHtml(story.title)}</div>
                    <div style="font-size:0.8rem; color:#999; margin-bottom:8px;">${escapeHtml(story.timestamp)}</div>
            ...
            <div class="history-detail" style="display:none; border-top:1px solid #eee; padding-top:10px; margin-top:10px; font-size:0.95rem; line-height:1.5;">
                <div style="background:#f9f9f9; padding:10px; border-radius:5px; margin-bottom:10px;">
                    <strong>📋 設定清單：</strong><br>${escapeHtml(listContent).replace(/\n/g, '<br>')}
                </div>
                
                <p><strong>📐 三幕劇結構：</strong><br>${escapeHtml(threeActContent).replace(/\n/g, '<br>')}</p>
                <hr style="border:0; border-top:1px dashed #ddd;">
                
                <p><strong>⭕ 故事圈：</strong><br>${escapeHtml(circleContent).replace(/\n/g, '<br>')}</p>
                <hr style="border:0; border-top:1px dashed #ddd;">
                
                <p><strong>📖 大綱：</strong><br>${escapeHtml(outlineContent).replace(/\n/g, '<br>')}</p>
                <hr style="border:0; border-top:1px dashed #ddd;">
                
                <p><strong>📊 分析：</strong><br>${escapeHtml(analysisContent).replace(/\n/g, '<br>')}</p>
                
                <div style="margin-top: 15px; text-align: center; color: #bfaea8; font-size: 0.8rem; border-top: 1px dashed #eee; padding-top: 10px;">
                    Generated by ${escapeHtml(modelName)}
                </div>
